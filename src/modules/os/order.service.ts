...
export class OsService {
  ...
  async editServicesDto(order_id: string, budget_id: string, dto: EditDealernetServiceDTO[]): Promise<UpdateDealernetOsDTO> {
    ...
    for (const item of dto) {
      const service_pps = await this.petroplay.order.findOrderBudgetService(order_id, budget_id, item.service_id);
      ...
      if (item?.status) {
        const Produtos: UpdateDealernetServiceProductDTO[] = [];
        const aux_products = dealernet_order_service?.Produtos || [];
        for (const product of aux_products) {
          Produtos.push({
            Chave: product.Chave,
            StatusExecucao: item?.status,
  ...
  async editServicesSchema(order_id: string, budget_id: string, dto: EditDealernetServiceDTO[]): Promise<string> {
    const order = await this.petroplay.order.findById(order_id, ['consultant', 'os_type', 'budgets']);
    if (!order) throw new NotFoundException(`Order '${order_id}' not found`);
    ...
    const editServicesDto = await this.editServicesDto(order_id, budget_id, dto);
    return this.dealernet.order.updateOsXmlSchema(integration.dealernet, editServicesDto);
  }
  async editServices(order_id: string, budget_id: string, dto: EditDealernetServiceDTO[]): Promise<DealernetOrderResponse> {
    const order = await this.petroplay.order.findById(order_id, ['consultant', 'os_type', 'budgets']);
    if (!order) throw new NotFoundException(`Order '${order_id}' not found`);
    ...
    const result = await this.dealernet.order
      .updateOs(integration.dealernet, editServicesDto)
      .then((response) => {
        return response;
      })
      .catch((error) => {
        this.context.setWarning('Erro ao editar serviço a Ordem de Serviço');
        Logger.error(`Erro ao editar serviços da ordem ${order_id}`, error, 'OsService');
        throw new BadRequestException('Erro ao editar serviço a ordem', {
          description: error?.message ?? 'Não foi possível editar o serviço a ordem',
        });
      });
